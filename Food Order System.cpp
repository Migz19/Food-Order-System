		...
		case 6:
			...
			if (validity())
			{
				...
				else
				{
					cout << "I'm glad we were able to help you today and we so appreciate your business. If you have the time and wouldn't mind, we'd love it if you shared this experience on [FCIS Order System]" << endl << "If you don't want just press enter :)" << endl; feedback();
					feedback();
					cout << endl << "Thanks for your feedback" << endl << "press y/Y to get back to main menu and N/n to extit the program " << endl;
					if (validity())
					{
						continue;
					}
...
float discountFunction() {
	...
	if (validpromo == false) {
		cout << "wrong promo code did you make a typo?";
		if (validity()) {
...
void recommended_meal()
{
	...
	for (int i = 0; i < 10; i++)
	{
		if (price >= meals[i].price && price >= 10 && price <= 100)
		{
			cout << "----------------------------------------------\n";
